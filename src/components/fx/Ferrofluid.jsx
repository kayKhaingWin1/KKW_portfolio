...
const Ferrofluid = ({
  ...
  // Mount-only: builds the WebGL context once. Visual props (colors, opacity,
  // speed, ...) are pushed into the existing uniforms by the effect below
  // instead of tearing down and recreating the renderer, so a theme toggle
  // (which produces a new `colors` array reference every render) doesn't
  // cause a visible rebuild/flash of the background.
  useEffect(() => {
    ...
    // Mount-only: intentionally excludes visual props so changing them (e.g. a
    // theme toggle) never tears down/recreates the WebGL context. See the
    // sync effect below, which pushes updates into the live uniforms instead.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dpr, mouseInteraction, mouseDampening]);

  // Push visual prop changes into the already-running program's uniforms in
  // place, so theme toggles (new `colors` array + opacity/mixBlendMode swap)
  // update instantly instead of rebuilding the whole renderer.
  useEffect(() => {
